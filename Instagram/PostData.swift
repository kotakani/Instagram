...
class PostData: NSObject {
    ...
    init(document: QueryDocumentSnapshot) {
        ...
        if let commentData = postDic["comments"] as? [[String : String]] {
            self.commentData = commentData
        }
    }
    
    override var description: String {
        return "PostData: name=\(name); caption=\(caption); date=\(date); likes=\(likes.count); commentData=\(commentData); id=\(id);"
    }
}
